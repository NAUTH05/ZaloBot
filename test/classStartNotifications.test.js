...
test("enabled feature sends once and duplicate evaluation is ignored", async (t) => {
    ...
    assert.match(sent[0].message, /ĐẾN GIỜ HỌC RỒI BẠN ƠI/);
    assert.match(sent[0].message, /B301\\_Ecommerce LAB - Cơ sở I/);
    assert.equal(Object.keys(readState(stateFilePath).events).length, 1);
});

test("multiple classes and different students are delivered independently", async (t) => {
    const sent = [];
    const schedules = {
    ...
    assert.deepEqual(sent.map((item) => item.studentId).sort(), ["123456789", "123456789", "987654321"]);
});
