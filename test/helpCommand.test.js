...
test("lệnh /help tạo tin nhắn hướng dẫn thành công và không bị lỗi ReferenceError", () => {
    const mainPath = path.join(__dirname, "../main.js");
    const code = fs.readFileSync(mainPath, "utf8");

    const helpHandlerMatch = code.match(/bot\.onText\(\/\^\\\/help\\s\*\$\/i,\s*\((?:msg|\w+)\)\s*=>\s*\{([\s\S]*?)\n\}\);/);
    ...
    const fn = new Function("msg", "sendMessage", handlerBody);
    fn({ chat: { id: 123456 } }, fakeSendMessage);
