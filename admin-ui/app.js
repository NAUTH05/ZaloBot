...
function timeChips(times = []) { return times.length ? `<div class="chip-list">${times.map((item) => `<span class="time-chip"><strong>#${escapeHtml(item.id)} · ${escapeHtml(item.time)}</strong><small>${escapeHtml(formatDate(item.createdAt))}</small></span>`).join("")}</div>` : `<span class="muted">Chưa đăng ký giờ</span>`; }
...
function renderOverview() {
  ...
  $("#metricGrid").innerHTML = [
    ...
    ["Nhận lịch", activeSubs, `${workspace.subscriptions.length} total records`, "notifications"],
...
function renderUsers() {
  ...
  $("#userRows").innerHTML = users.map((user) => {
    ...
    return `<tr><td><strong>${escapeHtml(user.displayName)}</strong><code>${escapeHtml(user.userId)}</code></td><td>${user.chats.map((chat) => `<div class="context-line">${badge(chat.chatType, chat.chatType === "group" ? "info" : "neutral")} ${escapeHtml(chat.chatName)} <code>${escapeHtml(chat.chatId)}</code></div>`).join("")}</td><td>${user.studentIds.length ? user.studentIds.map((id) => badge(id, "info")).join(" ") : `<span class="muted">Chưa lưu MSSV</span>`}</td><td>${timeChips(times)}</td><td>${badge(user.status, statusTone(user.status))} ${badge(user.notificationsEnabled ? "Notify on" : "Notify off", user.notificationsEnabled ? "success" : "neutral")}</td><td><button class="table-action" data-user="${escapeHtml(user.userId)}">Quản lý</button></td></tr>`;
  }).join("") || emptyRow(6, "Không tìm thấy user phù hợp.");
...
function renderGroups() {
  ...
  $("#groupCards").innerHTML = groups.map((group) => `<article class="group-card"><div class="card-heading"><div><span class="eyebrow">${escapeHtml(group.typeSource)}</span><h2>${escapeHtml(group.displayName)}</h2><code>${escapeHtml(group.chatId)}</code></div>${badge(group.status, statusTone(group.status))}</div><div class="group-stats"><span><strong>${group.memberCount}</strong> members</span><span><strong>${group.studentIds.length}</strong> MSSV</span><span><strong>${group.enabledSubscriptionCount}</strong> enabled</span></div><div class="member-preview">${group.members.slice(0, 6).map((member) => `<div><strong>${escapeHtml(member.displayName)}</strong><small>${escapeHtml(member.studentIds.join(", ") || "Chưa có MSSV")}</small></div>`).join("") || `<span class="muted">Chưa có member record</span>`}</div><button class="secondary full" data-chat-detail="${escapeHtml(group.chatId)}">Xem toàn bộ group</button></article>`).join("") || `<div class="empty-state panel">Không có group phù hợp.</div>`;
...
function renderDirectory() {
  ...
  $("#directoryRows").innerHTML = chats.map((chat) => `<tr><td><strong>${escapeHtml(chat.displayName)}</strong><code>${escapeHtml(chat.chatId)}</code></td><td>${badge(chat.chatType, chat.chatType === "group" ? "info" : chat.chatType === "unknown" ? "warning" : "neutral")}<small class="block">${escapeHtml(chat.typeSource)}</small></td><td><code>${escapeHtml(chat.userId || "-")}</code></td><td>${badge(chat.status, statusTone(chat.status))}</td><td>${escapeHtml(chat.memberCount)} member(s)<small class="block">${escapeHtml(chat.studentIds.join(", ") || "Chưa có MSSV")}</small></td><td><small>Inbound: ${escapeHtml(formatDate(chat.lastInboundInteractionAt))}</small><small class="block">Success: ${escapeHtml(formatDate(chat.lastSuccessfulDeliveryAt))}</small></td><td><button class="table-action" data-chat-detail="${escapeHtml(chat.chatId)}">Quản lý</button></td></tr>`).join("") || emptyRow(7, "Không có chat phù hợp.");
...
function renderNotifications() {
  ...
  $("#notificationRows").innerHTML = list.map((item) => `<tr><td><strong>${escapeHtml(item.userDisplayName || item.userId || "Legacy record")}</strong><code>${escapeHtml(item.userId || "-")}</code><div>${badge(item.studentId || "No MSSV", item.studentId ? "info" : "neutral")} ${escapeHtml(item.studentName)}</div></td><td><strong>${escapeHtml(item.chatName)}</strong><code>${escapeHtml(item.chatId)}</code></td><td>${badge(item.chatType, item.chatType === "group" ? "info" : "neutral")}</td><td>${timeChips(item.notificationTimes)}</td><td>${badge(item.schema, item.schema === "current" ? "success" : "warning")} ${badge(item.notificationsEnabled ? "Enabled" : "Disabled", item.notificationsEnabled ? "success" : "neutral")}</td><td><button class="table-action" data-subscription="${escapeHtml(item.key)}">Quản lý</button></td></tr>`).join("") || emptyRow(6, "Không có đăng ký phù hợp.");
...
function renderLogs() {
  $("#systemLogList").innerHTML = (logs.system || []).map((item) => `<article class="stack-item"><strong>${escapeHtml(item.level)}</strong><p>${escapeHtml(item.message)}</p><small>${escapeHtml(formatDate(item.at))}</small></article>`).join("") || `<div class="empty-state">Không có system log.</div>`;
  $("#deliveryLogList").innerHTML = (logs.deliveryErrors || []).map((item) => `<article class="stack-item"><strong>${escapeHtml(item.displayName || item.chatId)}</strong><p>${escapeHtml(item.lastError?.message || "-")}</p><small>${escapeHtml(formatDate(item.lastError?.at))}</small></article>`).join("") || `<div class="empty-state">Không có delivery error.</div>`;
}
...
function openUser(userId) {
  ...
  showDialog(user.displayName, `<div class="detail-grid"><div class="detail"><span>User ID</span><code>${escapeHtml(user.userId)}</code></div><div class="detail"><span>MSSV</span><strong>${escapeHtml(user.studentIds.join(", ") || "Chưa có")}</strong></div><div class="detail"><span>First interaction</span><strong>${escapeHtml(formatDate(user.firstInteractionAt))}</strong></div><div class="detail"><span>Last activity</span><strong>${escapeHtml(formatDate(user.lastInteractionAt))}</strong></div></div><h3>Chat contexts</h3><div class="stack">${user.chats.map((chat) => `<article class="stack-item"><form class="userContextForm" data-user-chat="${escapeHtml(chat.chatId)}"><div class="form-grid"><label>Display name<input name="displayName" value="${escapeHtml(user.displayName)}" /></label><label>Member status<select name="status"><option value="active" ${chat.memberStatus === "active" ? "selected" : ""}>Active</option><option value="disabled" ${chat.memberStatus === "disabled" ? "selected" : ""}>Disabled</option><option value="removed" ${chat.memberStatus === "removed" ? "selected" : ""}>Removed</option></select></label></div><p>${escapeHtml(chat.chatType)} · chat ${escapeHtml(chat.status)} · <code>${escapeHtml(chat.chatId)}</code></p><div class="row-actions"><button type="submit">Lưu user</button><button type="button" data-open-chat="${escapeHtml(chat.chatId)}">Mở chat</button><button type="button" data-user-admin="${escapeHtml(chat.chatId)}">Make admin</button><button type="button" class="danger-text" data-user-remove="${escapeHtml(chat.chatId)}">Xóa khỏi chat</button></div></form></article>`).join("")}</div><h3>Subscriptions</h3><div class="stack">${user.subscriptions.map((item) => `<article class="stack-item horizontal"><div><strong>${escapeHtml(item.studentId || "No MSSV")} · ${escapeHtml(item.chatName)}</strong><p>${escapeHtml(item.notificationsEnabled ? "Enabled" : "Disabled")} · ${escapeHtml(item.notificationTimes.map((time) => time.time).join(", ") || "No times")}</p></div><button data-open-sub="${escapeHtml(item.key)}">Quản lý</button></article>`).join("") || `<div class="empty-state">Không có subscription.</div>`}</div>`);
...
function openSubscription(key) {
  ...
  showDialog(`${item.studentId || "Subscription"} · ${item.chatName}`, `<div class="detail-grid"><div class="detail"><span>User</span><strong>${escapeHtml(item.userDisplayName || item.userId || "Legacy")}</strong><code>${escapeHtml(item.userId || "-")}</code></div><div class="detail"><span>Chat</span><strong>${escapeHtml(item.chatName)}</strong><code>${escapeHtml(item.chatId)}</code></div><div class="detail"><span>Chat type</span><strong>${escapeHtml(item.chatType)}</strong></div><div class="detail"><span>Record updated</span><strong>${escapeHtml(formatDate(item.updatedAt))}</strong></div></div>${item.schema === "legacy" ? `<div class="warning-box">Bản ghi legacy không có userId rõ ràng; chỉ nên xem hoặc xóa.</div>` : `<form id="subscriptionMetaForm"><div class="form-grid"><label>MSSV<input name="studentId" value="${escapeHtml(item.studentId)}" /></label><label>Tên sinh viên<input name="studentName" value="${escapeHtml(item.studentName)}" /></label><label>Tên user<input name="userDisplayName" value="${escapeHtml(item.userDisplayName)}" /></label></div><button class="primary" type="submit">Lưu thông tin</button></form><h3>Giờ đăng ký</h3><div class="stack">${item.notificationTimes.map((time) => `<article class="stack-item horizontal"><div><strong>#${time.id} · ${escapeHtml(time.time)}</strong><small>Đăng ký ${escapeHtml(formatDate(time.createdAt))} · cập nhật ${escapeHtml(formatDate(time.updatedAt))}</small></div><div class="row-actions"><button data-time-edit="${time.id}">Sửa</button><button class="danger-text" data-time-remove="${time.id}">Xóa</button></div></article>`).join("") || `<div class="empty-state">Chưa có giờ đăng ký.</div>`}<button class="secondary" id="addTimeButton">Thêm giờ</button></div><div class="action-bar"><button data-sub-action="${item.notificationsEnabled ? "disable" : "enable"}">${item.notificationsEnabled ? "Tắt thông báo" : "Bật thông báo"}</button><button class="danger-text" data-sub-action="delete">Xóa subscription</button></div>`}`);
